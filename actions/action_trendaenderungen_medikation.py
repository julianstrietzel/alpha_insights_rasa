...
class ActionTrendanderungenMedikation(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if not change_date:
            dispatcher.utter_message("Bitte geben Sie ein Datum der relevanten Medikationsänderung an.")
            return []
        pretty_change_date = (
        ...
        query = f"""
                        ...
                        WHERE 
                            user_id = {user_id}
                            AND CAST(recorded_at AS timestamp) >= '{pd.to_datetime(change_date) - pd.Timedelta(2, 'M')}'
        ...
        sns.scatterplot(data=bp_data, x='Date_num', y='Systolic', hue='Event', style='Event', palette='deep',
                        markers=['o'], s=20, legend=False)
        sns.regplot(data=bp_data[bp_data['Event'] == False], x='Date_num', y='Systolic', scatter=False,
                    label='Trend vor Änderung', color='blue')
        sns.regplot(data=bp_data[bp_data['Event'] == True], x='Date_num', y='Systolic', scatter=False,
                    label='Trend nach Änderung', color='red')
        sys_avg_before = bp_data_before.mean()
        ...
        plt.axhline(y=sys_avg_before, color='blue', linestyle='--', label=f'Sys. Durch. vorher: {sys_avg_before:.1f}',
                    alpha=0.5)
        plt.axhline(y=sys_avg_after, color='red', linestyle='--', label=f'Sys.Durch. nachher: {sys_avg_after:.1f}', alpha=0.5)

        # Same for diastolic
        sns.scatterplot(data=bp_data, x='Date_num', y='Diastolic', hue='Event', style='Event',
                        palette=['green', 'orange'], markers=['o'], s=20, legend=False)
        sns.regplot(data=bp_data[bp_data['Event'] == False], x='Date_num', y='Diastolic', scatter=False,
                    label='Trend vor Änderung', color='green')
        sns.regplot(data=bp_data[bp_data['Event'] == True], x='Date_num', y='Diastolic', scatter=False,
                    label='Trend nach Änderung', color='orange')
        ...
        plt.axhline(y=dia_avg_before, color='green', linestyle='--', label=f'Dias. Durch. vorher: {dia_avg_before:.1f}',
                    alpha=0.5)
        plt.axhline(y=dia_avg_after, color='orange', linestyle='--', label=f'Dias. Durch. nachher: {dia_avg_after:.1f}',
                    alpha=0.5)

        # Add vertical line for the event date
        respective_id = bp_data[bp_data['Date'] <= change_date].iloc[-1]['Date_num']
        plt.axvline(x=respective_id, color='black', linestyle='--',
                    label='Änderungsdatum' + f' ({change_date.strftime("%d.%m.%Y")})', linewidth=1)
        ...
        """)
        dispatcher.utter_message(f"""In den Wochen davor lagen die Messungen zwischen {sys_min_before}/{dia_min_before} mmHg und {sys_max_before}/{dia_max_before} mmHg und hatten einen Durchschnitt von {sys_avg_before:.2f}/{dia_avg_before:.2f} mmHg.
