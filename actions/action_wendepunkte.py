...
class ActionWendepunkte(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        change_date, diastolic_span, pretty_change_date, resulti, since_date, systolic_span, typ, user_id, zeitspanne = init_method_run(
            tracker)
        ...
        def analyze_inflection_points(data, typ, span):
            ...
            ax.set_xticklabels(data['recorded_at'][range(0, len(data), 30)].dt.strftime('%Y-%m-%d'))
            ...
            dispatcher.utter_message(
                f"Die drei signifikanten Wendepunkte im systolischen Blutdruck liegen am {change_dates[0].strftime('%d. %B %Y')}, {change_dates[1].strftime('%d. %B %Y')} und {change_dates[2].strftime('%d. %B %Y')}:")
            summary = []
            for i, (segment_data, reg) in enumerate(segments):
                ...
                trend = "Aufwärtstrend" if reg.coef_[0] > 0.05 else "Abwärtstrend" if reg.coef_[
                                                                                          0] < -0.05 else "konstanten Verlauf"
                summary.append(
...
def init_method_run(tracker):
    ...
    return change_date, diastolic_span, pretty_change_date, resulti, since_date, systolic_span, typ, user_id, zeitspanne
