...
""" + db_schema + basic_information,
    ...
    async def execute_query(self, question: str, output_function: callable, stream: bool = True):
        thread = self.thread
        ...
        if stream:
            with self.client.beta.threads.runs.stream(
                    ...
                    event_handler=EventHandler(output_function=output_function, gpt_handler=self, stream=stream)
            ) as stream:
...
class EventHandler(AssistantEventHandler):
    ...
    def __init__(self, output_function: callable = None, gpt_handler=None, stream: bool = True):
